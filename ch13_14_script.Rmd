...
```{r fit_1}
x <- x %>% 
  mutate(ideology = as.numeric(ideology) - 1)
#she centered, oof

z_1 <- stan_glm(dvote ~ race + ideology, data = x, refresh = FALSE, family = "binomial")

#tidy() on any model is very helpful
```
...
```{r}
#Ideology Coefficient divided by four
-0.9/4

plogis(-.09 * 5) - plogis(-.09 * 4)
#almost the same! gang gang!
```
...
```
you are 35% less likely to vote democratic 
...
Intercept is represents black voters with a score of 1 (very liberal) on the ideology scale 

```{r}
plogis(5.2) 
#99% likely to vote dem. who woulda thunk it
```
...
```{r, cache = TRUE}
z_3 <- stan_glm(dvote ~ race + ideology + race:ideology, data = x, refresh = FALSE, family = "binomial")
...
it is the same, no interaction in the intercept really

* Interpret the coefficient of "raceWhite:c_ideology". Pages 226ff may also be helpful.
the added coefficient coefficient, for each additional
if other, ideology is acualy -.04 more same with white. ideology more impact on vote when white and other
...
```{r}
hist <- posterior_linpred(object = z_3, newdata = x, transform = TRUE)

#pulling for third row (person)
hist(hist[,3])
```
...
```{r}
histo <- posterior_predict(object = z_3, newdata = x, transform = TRUE)
